...
namespace CodeMonkey.Utils {
    ...
    public static class UtilsClass {
       ...
       public static string GetByFormatTextByIndex(string itemName, int index)
        {

            if (itemName !=null && itemName.Contains(','))
            {
                string[] itemData = itemName.Split(new char[] { ',' }); return itemData[index];
            }
        ...
        public static Vector3 GetObjectWorldPosition(Vector3 position)
        {
            Vector3 vec = GetMouseWorldPositionWithZ(position, Camera.main);
            vec.z = 0f;
            return vec;
        }
