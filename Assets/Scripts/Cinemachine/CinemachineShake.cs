...
public class CinemachineShake : MonoBehaviour
{
    ...
    public void FollowClean() { cinemachineVirtualCamera.Follow = null; }
    public void SetFollow(Transform transform) { cinemachineVirtualCamera.Follow = transform; }
    public void DefaultSetup() {
        CinemachineBasicMultiChannelPerlin cinemachineBasicMultiChannelPerlin = cinemachineVirtualCamera.GetCinemachineComponent<CinemachineBasicMultiChannelPerlin>();
        cinemachineBasicMultiChannelPerlin.m_AmplitudeGain = 0;
    }
    public void ResetForOnStartLevel(bool offOn) { gameObject.SetActive(offOn); }
    void Update()
    {
        if(shakeTimer > 0)
        {
